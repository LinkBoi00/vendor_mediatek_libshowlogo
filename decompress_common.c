...
int decompress_logo(void *in, void *out, int inlen, int outlen)
{
    ...
    do {
        SLOGD("[decompress_logo %s %d] in do 1 <= 0\n",__FUNCTION__,__LINE__);
        strm.avail_in = inlen;
        ...
        do {
             SLOGD("[decompress_logo %s %d] in do 2\n",__FUNCTION__,__LINE__);
            strm.avail_out = outlen;
            strm.next_out = (Bytef*)out;
            ret = inflate(&strm, Z_NO_FLUSH);
            SLOGD("[decompress_logo %s %d] in do 2 ret =  %d\n",__FUNCTION__,__LINE__, ret);
            switch (ret) {
            case Z_NEED_DICT:
            ...
            SLOGD("[decompress_logo %s %d] do 2 have = %d \n",__FUNCTION__,__LINE__,have);
